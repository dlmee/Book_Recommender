...
#Troubleshooting
featcount <- ncol(dfp3)
P <- matrix(sample(1:featcount, replace = TRUE)/10, nrow=nrow(dfp3), ncol=ncol(dfp3))
Q <- matrix(sample(1:featcount, replace = TRUE)/10, nrow=ncol(dfp3), ncol=nrow(dfp3))
P[1,] %*% Q[,1]
dfp3[1,1]
loss <- dfp3[1,1] - P[1,] %*% Q[,1]
loss <- loss[1,1]
P[1,1] <- P[1,1] + .001 * (2 * 3.65 * Q[1,1] - .02 * P[1,1])
set.seed(9)
if (dfp3[1,1]>0){
  print("hello")
}
featcount <- ncol(dfp3)
featcount <- 1:featcount


# Now use Matrix Factorization to fill in missing scores. 
factorization <- function(tbf, steps, alpha, beta){
  featcount <- ncol(tbf) #Here's my problem! I'm looping through a single value. 
...
factored <- factorization(dfp3, 1000, .001, .02)
